...
namespace wukong::utils {
    ...
    UniqueRedisReply Redis::safeRedisCommand(const char *format, ...) {
        ...
        UniqueRedisReply reply =
                wrapReply((redisReply *) redisvCommand(context, format, args));
        va_end(args);
        return reply;
    }
    ...
    bool Redis::sismember(const std::string &key, const std::string &value) {
        auto reply =
                safeRedisCommand("SISMEMBER %s %s", key.c_str(), value.c_str());
    ...
    void Redis::sadd(const std::string &key, const std::string &value) {
        auto reply =
                safeRedisCommand("SADD %s %s", key.c_str(), value.c_str());
        if (reply->type == REDIS_REPLY_ERROR) {
